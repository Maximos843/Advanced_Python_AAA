...
def winner_positions(board: list[list[str]]) -> [str, list[tuple[int]]]:
    ...
    for player in ['USER', 'COMPUTER']:
        ...
        for i in range(3):
            if board[0][i] == board[1][i] and board[1][i] == board[2][i] and board[1][i] == symb:
...
async def game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    # check the correctness of user the move, whether there is an empty place there
    ...
    # fill in our user's turn and check if he won at this stage
    ...
    # choose the position of the bot and if there are no places left, then draw
    ...
    # fill in the position of the bot and see if there is a winner, respectively
